...
export default function ChatPage() {
  ...
  const [useImageGeneration, setUseImageGeneration] =
  ...
  const currentModelSupportsImageGeneration = () => {
    const spec = availableModels.find((m) => m.value === selectedModel);
    return spec?.supportsImageGeneration ?? false;
  };
  ...
  React.useEffect(() => {
    ...
    // Reset image generation when switching to a model that doesn't support it
    if (!currentModelSupportsImageGeneration() && useImageGeneration) {
      setUseImageGeneration(false);
    }
  }, [selectedModel, useWebSearch, useImageGeneration]);
    ...
    try {
      ...
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          useImageGeneration:
          useImageGeneration && currentModelSupportsImageGeneration(),
  ...
  const handleRegenerate = React.useCallback(
      ...
      try {
        ...
        const response = await fetch("/api/chat", {
          ...
          body: JSON.stringify({
            ...
            useImageGeneration:
            useImageGeneration &&
            (currentModelSpec?.supportsImageGeneration ?? false),
    ...
    [selectedModel, availableModels, useWebSearch,useImageGeneration, userPreferences]
